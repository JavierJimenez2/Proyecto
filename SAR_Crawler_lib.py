...
class SAR_Wiki_Crawler:
    ...
    def parse_wikipedia_textual_content(self, text: str, url: str) -> Optional[Dict[str, Union[str, List]]]:
        ...
        if partes:
            diccionario['title'] = partes.group(
                'title')  # Si no esta vacio asigna el match de titulo con el titulo del diccionario
            diccionario["summary"] = partes.group(
            ...
            for i in range(len(start_indices)):  # Para cada indice de inicio de seccion
                ...
                if (i + 1) >= len(
                        start_indices):  # Si el indice siguiente es mayor que la longitud de la lista de indices
                ...
                nombre, texto, sub_secciones = dic['name'], dic['text'], dic[
                    'rest']  # Sacamos el nombre, texto y subsecciones de cada match
